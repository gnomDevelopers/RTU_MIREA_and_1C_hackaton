      ...
      <div v-if="!isGroupGradesEmpty && tableType === 0 && isSelectedDiscipline" class="flex flex-row self-stretch items-start flex-wrap-0">
        ...
        <div class="overflow-x-scroll scrollable-table cursor-default">
          <table class="w-auto no-x-border table-decorate">
            <thead>
              <tr>
                <th v-for="item in getGroupGrades[0].grades" class="w-16 min-w-10 h-9">{{ item.date.slice(0, 5) }}</th>
              </tr>
            </thead>
            <tbody>
        ...
        <table class="cursor-default table-decorate">
          <thead>
            <tr>
              ...
              <th v-for="item in getGroupGrades[0].grades">{{ item.date.slice(0, 5) }}</th>
              <th>Ср.балл</th>
              <th>GPA</th>
              <th class=" bg-transparent border-none border-transparent cursor-pointer">
                <svg @click="sortByGPA" width="33" height="21" viewBox="0 0 33 21" fill="none" xmlns="http://www.w3.org/2000/svg">
