...
import { ROLES_NAME, ROLES_SET_PRORECTOR, ROLES_SET_DECAN_TO_PREPOD, ROLES_SET_DECAN_TO_STUDENT } from '@/helpers/constants';
export default{
  props:{
    id:{
      type: Number,
  ...
  computed:{
    ...
    getButtonText(){
      ...
      if(this.userInfoStore.role === 1){
        ...
        if(this.role === 2) return 'Понизить до ' + ROLES_NAME[rolesSum - this.role];
        if(this.role === 4) return 'Повысить до ' + ROLES_NAME[rolesSum - this.role]; 
      }
      if(this.userInfoStore.role === 2){
        if(ROLES_SET_DECAN_TO_PREPOD.includes(this.role)){
          rolesSum = ROLES_SET_DECAN_TO_PREPOD.reduce((sum, item) => sum += item);
          if(this.role === 3) return 'Понизить до ' + ROLES_NAME[rolesSum - this.role];
          if(this.role === 4) return 'Повысить до ' + ROLES_NAME[rolesSum - this.role]; 
        }
        else{
          rolesSum = ROLES_SET_DECAN_TO_STUDENT.reduce((sum, item) => sum += item);
          if(this.role === 5) return 'Понизить до ' + ROLES_NAME[rolesSum - this.role];
          if(this.role === 6) return 'Повысить до ' + ROLES_NAME[rolesSum - this.role]; 
        }
      }
